...
def evaluate_summarization(model, tokenizer, dataset="cnn_dailymail", max_tokens=512, device=device):
    ...
    results = test_data.map(generate_summary, batched=True, batch_size=32)
    ...
    return rouge_output, eval_scores

model_names = ["gpt2", "gpt2-medium", "gpt2-large"]
for model_name in model_names[:1]:
    model, tokenizer = get_model(model_name)
    results = evaluate_summarization(model, tokenizer)
    with open(f"./results/{model_name}-summ.json", 'w') as f:
        json.dump(results, f)